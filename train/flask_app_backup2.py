...
def get_data():
    ...
    msg = "Your name is Zen and you're a mental health counselor. Please have a conversation with your patient and provide them with a helpful response to their concerns."
    
    for i in tqdm(range(len(df))):
        if not pd.isna(df.Zen3[i]):
            continue
        q = df.orig_prompt[i].split(msg)[1].strip()[5:]
        a = re.findall(r'ASSISTANT: (.*?)</s>', q, re.DOTALL)[-1]
        q = q.split(a)[0].strip()
        ind = q.rfind("ASSISTANT:")
        q = q[:ind] + q[ind + len("ASSISTANT:"):]
        q = q.strip()
        ...
        if isinstance(output_stream, str):
            ...
            answer = stream_output(output_stream)
